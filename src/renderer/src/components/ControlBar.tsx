...
export default function ControlBar({ onShowInvite }: { onShowInvite: () => void }) {
  ...
  const toggleMic = () =>
    guard(async () => {
      if (!s.local.mic) return meetingClient.enableMic(settings?.preferredMicId ?? null)
      return meetingClient.setMicMuted(!s.micMuted)
    })
  const toggleCam = () => guard(() => (s.local.camera ? meetingClient.disableCamera() : meetingClient.enableCamera(settings?.preferredCameraId ?? null)))
  const toggleShare = () => (s.sharing ? void guard(() => meetingClient.stopScreenShare()) : setPicker(true))

  const leave = () =>
    guard(async () => {
      if (isHost) {
        await meetingClient.closeRoom().catch(() => undefined)
        await window.sjting.host.stop()
      }
  ...
  return (
    <>
      <div className="flex items-center gap-2 border-t border-slate-800 bg-slate-950/90 px-4 py-3">
        <Ctl on={!!s.local.mic && !s.micMuted} danger={!!s.local.mic && s.micMuted} label={s.micMuted ? '마이크 켜기' : '마이크 끄기'} onClick={() => void toggleMic()}>
          {s.local.mic && !s.micMuted ? <Mic size={18} /> : <MicOff size={18} />}
        </Ctl>
        <Ctl on={!!s.local.camera} label={s.local.camera ? '카메라 끄기' : '카메라 켜기'} onClick={() => void toggleCam()}>
          {s.local.camera ? <Video size={18} /> : <VideoOff size={18} />}
        </Ctl>
        ...
        {isHost && (
          <>
            <Ctl label="초대코드" onClick={onShowInvite}>
              <Link2 size={18} />
            </Ctl>
            ...
            <Ctl label="전체 음소거" onClick={() => void guard(() => meetingClient.muteAll())}>
              <VolumeX size={18} />
            </Ctl>
          </>
        )}
